...
def build_bills_bimonthly(bills_path: Path, meter_labels: pd.DataFrame | None = None) -> pd.DataFrame:
    """모든 청구를 **2개월 단위**로 통일.
    ...
    meter_norm = meter_labels.copy() if meter_labels is not None else pd.read_csv(
        PROJECT_ROOT / "data" / "billing" / "meter_match.csv",
        encoding="utf-8-sig",
        dtype={"고객번호": str},
    )[["고객번호", "역명"]]
    ...
    # 역명 join (사람이 보기 쉽게)
    labels = meter_labels if meter_labels is not None else _load_meter_to_station()
