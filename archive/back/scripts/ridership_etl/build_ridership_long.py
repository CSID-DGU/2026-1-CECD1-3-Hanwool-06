...
Inputs:
    data/raw/ridership/seoul_api/raw/CARD_SUBWAY_MONTH_*.csv  (mixed encoding)
    data/billing/meter_match.csv                             (for match validation)

Output:
    data/processed/ridership_long.csv          (한글 헤더, normalized)
    data/processed/ridership_match_report.json (매칭 진단)
"""
...
BILLING_ETL_DIR = PROJECT_ROOT / "back" / "scripts" / "billing_etl"
if str(BILLING_ETL_DIR) not in sys.path:
    sys.path.insert(0, str(BILLING_ETL_DIR))

from postprocess_bills import parse_station_meter  # reuse


DEFAULT_RAW_DIR = PROJECT_ROOT / "data" / "raw" / "ridership" / "seoul_api" / "raw"
DEFAULT_METER_MATCH = PROJECT_ROOT / "data" / "billing" / "meter_match.csv"
DEFAULT_OUT_CSV = PROJECT_ROOT / "data" / "processed" / "ridership_long.csv"
DEFAULT_REPORT = PROJECT_ROOT / "data" / "processed" / "ridership_match_report.json"
