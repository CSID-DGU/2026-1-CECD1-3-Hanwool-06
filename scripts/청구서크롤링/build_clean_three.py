...
출력 (모두 Processed_data/ 루트, UTF-8-BOM + 한글 헤더):
  1) 일일사용량.csv : 미터 × 날짜 → 일사용량
  2) 청구서.csv     : 미터 × 월 → 월사용량 (격월 청구는 2달로 분할해서 월별 통일)
  3) 승하차.csv     : (역, 호선) × 날짜 → 승하차
...
DEFAULT_DAILY_IN = PROCESSED / "Daily_Data" / "daily_long.csv"
DEFAULT_BILLS_IN = PROCESSED / "Bill_Data" / "bills_clean.csv"
DEFAULT_RIDERS_IN = PROCESSED / "승하차데이터" / "ridership_long.csv"

OUT_DAILY = PROCESSED / "일일사용량.csv"
OUT_BILLS = PROCESSED / "청구서.csv"
OUT_RIDERS = PROCESSED / "승하차.csv"
...
def _load_meter_to_station() -> pd.DataFrame:
    ...
    """
    m = pd.read_csv(
        PROCESSED / "Bill_Data" / "mkey_station_map.csv",
        encoding="utf-8-sig",
        dtype={"고객번호": str},
...
def main() -> int:
    ...
    print("[1/3] 일일사용량.csv ...", flush=True)
    ...
    print("[2/3] 청구서.csv (2개월 단위 통일) ...", flush=True)
